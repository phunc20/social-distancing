...
ap.add_argument("-i", "--input", type=str, default="street-views/street-view-11.jpg",
...
while True:
    # Note that in order to correctly update the circular points that we draw on win_draw,
    # we must put cv2.imshow() inside the while loop and as well as right after it.
    ...
    if cv2.waitKey(33) & 0xFF == ord('q'):
        ...
        # It's said to be not good to use exit(), but
        # I simply find that wrapping the current code inside a main() function makes it extremely unclear,
        # especially considering that some part of the code must be separated far from its original location, making it hard to read
        exit()
    elif count >= 4:
        break
cv2.imshow(win_draw, tmp)
...
if len(results) >= 2:
    ...
    #D = dist.cdist(pieds, pieds, metric="euclidean")
...
for (i, (prob, bbox, centroid, pied)) in enumerate(results):
    ...
    #cv2.circle(warped1, pied, radius=0, color=color, thickness=20)
    cv2.circle(warped1, (pX, pY), radius=0, color=color, thickness=20)
...
cv2.moveWindow(win2, resized.shape[1]+2*borderpx, y_tl)
cv2.imshow(win2, graffiti)

win3 = "BEV"
cv2.namedWindow(win3)
cv2.moveWindow(win3, resized.shape[1]*2+4*borderpx, y_tl)
cv2.imshow(win3, warped1)
